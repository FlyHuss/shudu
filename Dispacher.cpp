...
void typename DispacherT<T>::CallbackFunc(shared_ptr<::google::protobuf::Message> msg_ptr){//在传递参数的时候就自动向下转型了，变成了具体的派生类型
	shared_ptr<::google::protobuf::Message> msg_ptrT((T*)msg_ptr);
...
//而不再需要字符串判断.
template<typename T>
void ProtoMessageDispacher::RegisterCallback(const ::google::protobuf::Descriptor* descriptor,std::function<void(T*)> cb){
...
//用来查找具体Message对应的DispacherT并调用。说了第三个类是来使用第一、第二个的。不需要用户调用第一第二的方法
